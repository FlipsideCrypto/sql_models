...
flashloan AS(
    SELECT
        DISTINCT block_id,
        block_timestamp,
        event_inputs:asset::string AS aave_market,
        event_inputs:amount AS flashloan_quantity, --not adjusted for decimals
        event_inputs:initiator::string AS initiator_address,
        event_inputs:target::string AS target_address,
        event_inputs:premium AS premium_quantity,
    ...
    WHERE 1=1
        ...
        AND event_name = 'FlashLoan' --this is a deposit
        AND tx_succeeded = TRUE --excludes failed txs
    --LIMIT 100
)

SELECT
    DISTINCT flashloan.block_id,
    flashloan.block_timestamp,
    underlying.aave_token AS atoken,
    prices.symbol AS token_symbol,
    --atokens.project_name AS atoken_symbol,
    flashloan.flashloan_quantity / POW(10,prices.decimals) AS flashloan_amount,
    flashloan.flashloan_quantity * prices.token_price / POW(10,prices.decimals) AS flashloan_amount_usd,
    flashloan.premium_quantity / POW(10,prices.decimals) AS premium_amount,
    flashloan.premium_quantity * prices.token_price / POW(10,prices.decimals) AS premium_amount_usd,
    --prices.decimals AS aux_prices_decimals,
    flashloan.initiator_address AS initiator,
    flashloan.target_address AS target_address,
    flashloan.tx_id,
    flashloan.aave_version
FROM
    flashloan
    LEFT JOIN prices
        ON flashloan.block_timestamp = prices.block_timestamp
        AND flashloan.aave_market = prices.token_address
    LEFT JOIN underlying
