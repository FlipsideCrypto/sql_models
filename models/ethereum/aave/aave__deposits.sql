...
prices AS (
    SELECT
        ...
        CASE WHEN eth_prices.decimals IS NULL THEN 18 ELSE eth_prices.decimals END AS decimals, -- decimals can be retrieved from the code at dex_swaps
        eth_prices.symbol,
...
SELECT
    ...
    deposits.deposit_quantity / POW(10,prices.decimals) AS issued_atokens,
    deposits.deposit_quantity * prices.token_price / POW(10,prices.decimals) AS supplied_usd,
    --prices.decimals AS aux_prices_decimals,
    prices.token_address AS supplied_contract_address,
...
FROM
    deposits
    LEFT JOIN prices
        ON deposits.block_timestamp = prices.block_timestamp
        AND deposits.aave_market = prices.token_address
    LEFT JOIN underlying
