    ...
    SELECT
        ...
        CASE
            WHEN COALESCE(event_inputs:collateralAsset::string,event_inputs:_collateral::string) = '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee'
                THEN '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
                ELSE COALESCE(event_inputs:collateralAsset::string,event_inputs:_collateral::string)
              END AS collateral_asset,
        CASE
            WHEN COALESCE(event_inputs:debtAsset::string,event_inputs:_reserve::string) = '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee'
                THEN '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
                ELSE COALESCE(event_inputs:debtAsset::string,event_inputs:_reserve::string)
              END AS debt_asset,
        COALESCE(event_inputs:debtToCover,event_inputs:_purchaseAmount) AS debt_to_cover_amount, --not adjusted for decimals
        COALESCE(event_inputs:liquidatedCollateralAmount,event_inputs:_liquidatedCollateralAmount) AS liquidated_amount,
        COALESCE(event_inputs:liquidator::string,event_inputs:_liquidator::string) AS liquidator_address,
        COALESCE(event_inputs:user::string,event_inputs:_user::string) AS borrower_address,
...
SELECT
    ...
    liquidation.liquidated_amount /
        POW(10,COALESCE(coalesced_prices.decimals,backup_prices.decimals,prices_daily_backup.decimals,decimals_backup.decimals,18)) AS liquidated_amount,
    liquidation.liquidated_amount * COALESCE(coalesced_prices.coalesced_price,backup_prices.price,prices_daily_backup.avg_daily_price) /
        POW(10,COALESCE(coalesced_prices.decimals,backup_prices.decimals,prices_daily_backup.decimals,decimals_backup.decimals,18)) AS liquidated_amount_usd,
    LOWER(liquidation.debt_asset) AS debt_asset,
    LOWER(underlying_debt.aave_token) AS debt_aave_token,
    liquidation.debt_to_cover_amount /
        POW(10,COALESCE(coalesced_prices_debt.decimals,backup_prices_debt.decimals,prices_daily_backup_debt.decimals,decimals_backup_debt.decimals,18)) AS debt_to_cover_amount,
    liquidation.debt_to_cover_amount * COALESCE(coalesced_prices_debt.coalesced_price,backup_prices_debt.price,prices_daily_backup_debt.avg_daily_price) /
        POW(10,COALESCE(coalesced_prices_debt.decimals,backup_prices_debt.decimals,prices_daily_backup_debt.decimals,decimals_backup_debt.decimals,18)) AS debt_to_cover_amount_usd,
    ...
    COALESCE(coalesced_prices_debt.symbol,backup_prices_debt.symbol,prices_daily_backup_debt.symbol,REGEXP_REPLACE(l.address,'AAVE.*: a','')) AS debt_token_symbol,
    'ethereum' AS blockchain
FROM
    liquidation
    LEFT JOIN coalesced_prices
        ON LOWER(liquidation.collateral_asset) = LOWER(coalesced_prices.token_contract)
        AND liquidation.aave_version = coalesced_prices.aave_version
        AND date_trunc('hour',liquidation.block_timestamp) = coalesced_prices.coalesced_hour
    LEFT JOIN backup_prices
        ON LOWER(liquidation.collateral_asset) = LOWER(backup_prices.token_address)
        AND date_trunc('hour',liquidation.block_timestamp) = backup_prices.hour
    LEFT JOIN prices_daily_backup
        ON LOWER(liquidation.collateral_asset) = LOWER(prices_daily_backup.token_address)
        AND date_trunc('day',liquidation.block_timestamp) = prices_daily_backup.block_date
    LEFT JOIN underlying
        ON LOWER(liquidation.collateral_asset) = LOWER(underlying.token_contract)
        AND liquidation.aave_version = underlying.aave_version
    LEFT JOIN decimals_backup
        ON LOWER(liquidation.collateral_asset) = LOWER(decimals_backup.token_address)
-- need to join twice to cover both collateral asset and debt asset
    LEFT JOIN coalesced_prices AS coalesced_prices_debt
        ON LOWER(liquidation.debt_asset) = LOWER(coalesced_prices_debt.token_contract)
        AND liquidation.aave_version = coalesced_prices_debt.aave_version
        AND date_trunc('hour',liquidation.block_timestamp) = coalesced_prices_debt.coalesced_hour
    LEFT JOIN backup_prices AS backup_prices_debt
        ON LOWER(liquidation.debt_asset) = LOWER(backup_prices_debt.token_address)
        AND date_trunc('hour',liquidation.block_timestamp) = backup_prices_debt.hour
    LEFT JOIN prices_daily_backup AS prices_daily_backup_debt
        ON LOWER(liquidation.debt_asset) = LOWER(prices_daily_backup_debt.token_address)
        AND date_trunc('day',liquidation.block_timestamp) = prices_daily_backup_debt.block_date
    LEFT JOIN underlying AS underlying_debt
        ON LOWER(liquidation.debt_asset) = LOWER(underlying_debt.token_contract)
