...
FROM
    ...
    LEFT JOIN coalesced_prices
        ON LOWER(repay.aave_market) = LOWER(coalesced_prices.token_contract)
        AND repay.aave_version = coalesced_prices.aave_version
        AND date_trunc('hour',repay.block_timestamp) = coalesced_prices.coalesced_hour
    LEFT JOIN backup_prices
        ON LOWER(repay.aave_market) = LOWER(backup_prices.token_address)
        AND date_trunc('hour',repay.block_timestamp) = backup_prices.hour
    LEFT JOIN prices_daily_backup
        ON LOWER(repay.aave_market) = LOWER(prices_daily_backup.token_address)
        AND date_trunc('day',repay.block_timestamp) = prices_daily_backup.block_date
    LEFT JOIN underlying
        ON LOWER(repay.aave_market) = LOWER(underlying.token_contract)
