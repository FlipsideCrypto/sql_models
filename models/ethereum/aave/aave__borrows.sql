...
SELECT
    ...
    CASE WHEN borrow_rate_mode = 2 THEN 'Variable Rate' ELSE 'Stable Rate' END AS borrow_rate_mode,
...
FROM
    ...
    LEFT JOIN coalesced_prices
        ON LOWER(borrow.aave_market) = LOWER(coalesced_prices.token_contract)
        AND borrow.aave_version = coalesced_prices.aave_version
        AND date_trunc('hour',borrow.block_timestamp) = coalesced_prices.coalesced_hour
    LEFT JOIN backup_prices
        ON LOWER(borrow.aave_market) = LOWER(backup_prices.token_address)
        AND date_trunc('hour',borrow.block_timestamp) = backup_prices.hour
    LEFT JOIN prices_daily_backup
        ON LOWER(borrow.aave_market) = LOWER(prices_daily_backup.token_address)
        AND date_trunc('day',borrow.block_timestamp) = prices_daily_backup.block_date
    LEFT JOIN underlying
        ON LOWER(borrow.aave_market) = LOWER(underlying.token_contract)
    ...
    {{ref('silver_crosschain__address_labels')}} l 
        ON LOWER(underlying.aave_token) = l.address AND l.blockchain = 'ethereum' AND l.creator = 'flipside'
