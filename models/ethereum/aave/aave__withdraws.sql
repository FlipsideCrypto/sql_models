...
prices_hourly AS(
    ...
    FROM
        ...
        LEFT JOIN {{ ref('ethereum__token_prices_hourly') }}
        eth_prices
...
SELECT
    ...
    LOWER(
        withdraw.aave_market
    ) AS aave_market,
    LOWER(
        underlying.aave_token
    ) AS aave_token,
    withdraw.withdraw_amount / pow(
        10,
        COALESCE(
    ...
    withdraw.withdraw_amount * COALESCE(
...
FROM
    ...
    AND l.creator = 'flipside' qualify(ROW_NUMBER() over(PARTITION BY tx_id, event_index
ORDER BY
    block_id ASC)) = 1
