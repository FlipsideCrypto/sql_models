...
SELECT
  ...
  COALESCE(
    creator_fee / n_tokens,
    0
  ) AS creator_fee,
  CASE
    WHEN sale_price.symbol IS NULL THEN sale_price.contract_address
    ELSE sale_price.symbol
  END AS tx_currency
FROM
  token_transfers tt
  JOIN sale_price
  ON tt.tx_id = sale_price.tx_id
  LEFT OUTER JOIN platform_fee
  ON tt.tx_id = platform_fee.tx_id
  LEFT OUTER JOIN creator_fee
