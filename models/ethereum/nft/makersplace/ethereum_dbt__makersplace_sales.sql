...
WITH 
makersplace_metadata AS (
  SELECT
    token_id, 
    creator_address
  FROM {{ ref('ethereum__nft_metadata') }}
  WHERE contract_address = '0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756'
),
eth_transfers AS (
  SELECT 
    tx_id, 
  ...
  WHERE tx_id IN (
    ...
    WHERE input_method = '0xae77c237' 
      AND contract_address = '0x2a46f2ffd99e19a89476e2f62270e0a35bbf0756')

  AND symbol = 'ETH' AND amount > 0 AND origin_address = from_address
  {% if is_incremental() %}
...
nft_transfers AS (
  ...
  FROM 
  {{ ref('ethereum__events_emitted') }} ee
...
SELECT
  ...
  WHEN (seller = creator_address OR creator_address IS NULL) THEN amount * 0.85
  END AS creator_fee,
  'ETH' AS tx_currency
FROM 
eth_transfers et
