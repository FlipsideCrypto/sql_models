{{ 
  config(
    ...
    tags=['snowflake', 'ethereum', 'compound', 'compound_repayments']
  )
}}
...
SELECT 
  DISTINCT block_id,
    ...
    REGEXP_REPLACE(event_inputs:payer,'\"','') AS payer,
    CASE WHEN p.token_contract = '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' THEN NULL ELSE p.token_contract END AS repay_contract_address,
    CASE WHEN p.token_contract = '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' THEN 'ETH' ELSE p.symbol END AS repay_contract_symbol,
    event_inputs:repayAmount/pow(10,p.token_decimals) AS repayed_amount,
    event_inputs:repayAmount*p.token_price/pow(10,p.token_decimals) AS repayed_amount_usd,
    tx_id        
