{{ 
  config(
    ...
    tags=['snowflake', 'ethereum', 'compound', 'compound_borrows']
  )
}}
...
SELECT 
    DISTINCT block_id,
      ...
      CASE WHEN p.token_contract = '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' THEN NULL ELSE p.token_contract END AS borrows_contract_address,
      CASE WHEN p.token_contract = '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' THEN 'ETH' ELSE p.symbol END AS borrows_contract_symbol,
      ee.contract_addr AS ctoken, 
      ctoks.project_name AS ctoken_symbol,
      event_inputs:borrowAmount/pow(10,p.token_decimals) AS loan_amount, 
      event_inputs:borrowAmount*p.token_price/pow(10,p.token_decimals) AS loan_amount_usd,
      tx_id
...
LEFT OUTER JOIN
ctok_decimals d
    ON ee.contract_addr = d.ctok_address
WHERE 
  {% if is_incremental() %}
  block_timestamp >= getdate() - interval '2 days'
  {% else %}
