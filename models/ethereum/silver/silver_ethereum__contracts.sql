...
WITH base AS (
  ...
  WHERE
    <<<<<<< head address IS NOT NULL qualify(ROW_NUMBER() over(PARTITION BY address
  ORDER BY
    system_created_at DESC)) = 1 ======= address IS NOT NULL qualify(ROW_NUMBER() over(PARTITION BY address
  ORDER BY
