...
usd_swaps AS (
  SELECT
    ...
    CASE
      WHEN event_inputs :amount0In > 0 THEN event_inputs :amount0In * COALESCE(
        price0.price,
        backup0.price
      ) / power(10, COALESCE(d0.decimals, backup0.decimals))
      ELSE event_inputs :amount0Out * COALESCE(
        price0.price,
        backup0.price
      ) / power(10, COALESCE(d0.decimals, backup0.decimals))
    END AS amount_usd,
    CASE
      WHEN p.factory_address = '0xc0aee478e3658e2610c5f7a4a2e1777ce9e4f2ac' THEN 'sushiswap'
      ELSE 'uniswap-v2'
    END AS platform,
    event_index,
    CASE
      WHEN event_inputs :amount0In > 0 THEN 'IN'
      ELSE 'OUT'
    END AS direction
  FROM
    {{ ref('ethereum__events_emitted') }}
    s0
    LEFT JOIN {{ ref('ethereum__dex_liquidity_pools') }}
    p
    ON s0.contract_address = p.pool_address
    LEFT JOIN prices price0
    ON p.token0 = price0.token_address
    AND DATE_TRUNC(
      'hour',
      s0.block_timestamp
    ) = price0.hour
    LEFT JOIN prices_daily_backup backup0
    ON p.token0 = backup0.token_address
    AND DATE_TRUNC(
      'day',
      s0.block_timestamp
    ) = backup0.block_date
...
SELECT
  ...
  CASE
    WHEN event_inputs :amount1In > 0 THEN event_inputs :amount1In * COALESCE(
      price1.price,
      backup1.price
    ) / power(10, COALESCE(d1.decimals, backup1.decimals))
    ELSE event_inputs :amount1Out * COALESCE(
      price1.price,
      backup1.price
    ) / power(10, COALESCE(d1.decimals, backup1.decimals))
  END AS amount_usd,
  ...
  event_index,
  CASE
    WHEN event_inputs :amount1In > 0 THEN 'IN'
    ELSE 'OUT'
  END AS direction
FROM
  {{ ref('ethereum__events_emitted') }}
  ...
  ON p.token1 = price1.token_address
  AND DATE_TRUNC(
    'hour',
    s0.block_timestamp
  ) = price1.hour
  LEFT JOIN prices_daily_backup backup1
  ON p.token1 = backup1.token_address
  AND DATE_TRUNC(
    'day',
    s0.block_timestamp
  ) = backup1.block_date
...
v3_swaps AS (
  SELECT
    ...
    s.tx_id,
    CASE
      WHEN amount0_adjusted > 0 THEN amount0_adjusted
      ELSE NULL
    END AS amount_in,
    CASE
      WHEN amount0_adjusted <= 0 THEN -1 * amount0_adjusted
      ELSE NULL
    END AS amount_out,
    ...
    ind.event_index,
    CASE
      WHEN amount0_adjusted > 0 THEN 'IN'
      ELSE 'OUT'
    END AS direction
  FROM
    -- {{source('uniswapv3_eth','uniswapv3_swaps')}}
    {{ ref('uniswapv3__swaps') }}
    s
    LEFT JOIN {{ ref('ethereum__dex_liquidity_pools') }}
    dl
    ON s.pool_address = dl.pool_address
    LEFT JOIN prices p
    ON dl.token0 = p.token_address
    AND p.hour = DATE_TRUNC(
      'hour',
      block_timestamp
    )
    LEFT JOIN {{ ref('ethereum__events_emitted') }}
    ind
    ON s.pool_address = ind.contract_address
    AND s.tx_id = ind.tx_id
  WHERE
    (
      amount1_adjusted > 0
      OR amount0_adjusted > 0
    )
    AND platform = 'uniswap-v3'
  UNION
  SELECT
    ...
    s.tx_id,
    CASE
      WHEN amount1_adjusted > 0 THEN amount1_adjusted
      ELSE NULL
    END AS amount_in,
    CASE
      WHEN amount1_adjusted <= 0 THEN -1 * amount1_adjusted
      ELSE NULL
    END AS amount_out,
    sender AS from_address,
    recipient AS to_address,
    ...
    ind.event_index,
    CASE
      WHEN amount1_adjusted > 0 THEN 'IN'
      ELSE 'OUT'
    END AS direction
  FROM
...
swaps AS (
  ...
  FROM
    usd_swaps
  WHERE
    pool_address NOT IN ('0xdc6a5faf34affccc6a00d580ecb3308fc1848f22') -- stop-gap for big price swings, the actual solution adds an enormous amount of runtime
  UNION
  SELECT
    *
  FROM
  ...
  SELECT
    ...
    'IN' AS direction
  FROM
    {{ ref('ethereum_dbt__curve_swaps') }} C
  WHERE
...
FINAL AS (
  SELECT
    ...
    s.direction,
    CASE
      WHEN s.from_address <> s.to_address THEN l.address
      ELSE NULL
    END AS router
  FROM
    swaps s
...
SELECT
  ...
  router
FROM
  FINAL qualify(ROW_NUMBER() over(PARTITION BY tx_id, event_index, direction, token_address, amount_in, amount_out
ORDER BY
  block_timestamp DESC, router DESC nulls last)) = 1
