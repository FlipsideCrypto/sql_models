{{ 
  config(
    ...
    tags=['snowflake', 'polygon_silver', 'polygon_dbt_udm_events']
  )
}}
...
with base_tables as (
  ...
  {% if is_incremental() %}
        AND (record_metadata:CreateTime::int/1000)::timestamp >= (select max(system_created_at) from {{source('polygon_dbt', 'udm_events')}})
  {% endif %}
  )
