...
SELECT max(block_id) AS max_block, pool_name
...
reference_pool AS (
SELECT block_id, block_timestamp, bpd.pool_name, rune_e8
FROM {{source('thorchain', 'block_pool_depths')}}  bpd
JOIN max_pool_blocks mpb ON bpd.pool_name = mpb.pool_name AND bpd.block_id = mpb.max_block
WHERE
{% if is_incremental() %}
