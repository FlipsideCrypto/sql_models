{{ config(
  ...
  tags = ['snowflake', 'console', 'terra', 'top_holder_flows']
) }}
...
FROM recent_events
WHERE (from_segment = 'Top Holder' or to_segment = 'Top Holder')
...
CASE
    WHEN from_segment = 'Top Holder' THEN -(volume)
    WHEN to_segment = 'Top Holder' THEN volume
    END AS volume,
CASE
    WHEN from_segment = 'Top Holder' then tx_count
    ELSE 0
    END AS tx_outflow,
CASE
    WHEN to_segment = 'Top Holder' then tx_count
    ELSE 0
    END AS tx_inflow,
CASE
    WHEN from_segment = 'Top Holder' then address_count
    ELSE 0
    END AS address_outflow,
CASE
    WHEN to_segment = 'Top Holder' then address_count
    ELSE 0
    END AS address_inflow
from top_holder_flows
