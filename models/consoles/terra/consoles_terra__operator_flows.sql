{{ config(
  ...
  tags = ['snowflake', 'console', 'terra', 'operator_flows']
) }}
...
FROM recent_events
WHERE (event_from_label_type = 'operator' OR event_to_label_type = 'operator')
...
CASE
    WHEN from_segment = 'Operator' THEN -(volume)
    WHEN to_segment = 'Operator' THEN volume
    END AS volume,
CASE
    WHEN from_segment = 'Operator' then tx_count
    ELSE 0
    END AS tx_outflow,
CASE
    WHEN to_segment = 'Operator' then tx_count
    ELSE 0
    END AS tx_inflow,
CASE
    WHEN from_segment = 'Operator' then address_count
    ELSE 0
    END AS address_outflow,
CASE
    WHEN to_segment = 'Operator' then address_count
    ELSE 0
