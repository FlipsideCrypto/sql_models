{{ config(
  ...
  cluster_by = ['date'],
  tags = ['snowflake', 'mirror', 'console', 'mirror_terra_avg_gas_fee']
) }}

-- original by FlipoCrypto https://app.flipsidecrypto.com/velocity/queries/083e239d-bc88-4032-8a57-a77823737495
WITH jdata as (
...
, gas_tx as (
  ...
  WHERE TX_STATUS = 'SUCCEEDED' 
    AND (FEE[0]:amount[0] IS NOT NULL OR FEE[0]:amount IS NOT NULL) 
...
SELECT 
  ...
  base_masset, 
  avg_gas_usd,
  AVG(avg_gas_usd) OVER (PARTITION BY base_masset ORDER BY DATE ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) as "SEVEN_DMA"
FROM daily_avg
