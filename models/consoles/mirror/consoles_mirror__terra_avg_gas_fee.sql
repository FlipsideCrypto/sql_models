...
, daily_avg as (
  SELECT to_char(date, 'YYYY-MM-DD') as day, base_masset, avg(gas_usd) as avg_gas_usd
  FROM mirror_tx m INNER JOIN gas_conversion g ON m.tx_id = g.tx_id
  GROUP BY 1, 2
  ORDER BY 1
)

SELECT day, base_masset, avg_gas_usd,
  AVG(avg_gas_usd) OVER (PARTITION BY base_masset ORDER BY DAY ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) as "7_DMA"
FROM daily_avg
