-- original by cryptoicicle https://app.flipsidecrypto.com/velocity/queries/36abc0cb-5d90-4abf-82e9-93d76522c6d4
WITH jdata as (
...
SELECT date, base_masset, sum(count(distinct sender)) OVER (PARTITION BY base_masset ORDER BY date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_sum
FROM tx
GROUP BY 1, 2

UNION 

SELECT date, 'Total' as base_masset, sum(count(distinct sender)) OVER (ORDER BY date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_sum
FROM tx
GROUP BY 1, 2
ORDER BY 1