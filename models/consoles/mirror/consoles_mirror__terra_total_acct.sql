{{ config(
  ...
  cluster_by = ['date'],
  tags = ['snowflake', 'mirror', 'console', 'mirror_terra_total_acct']
) }}
...
SELECT 
  date, 
  base_masset, 
  sum(count(distinct sender)) OVER (PARTITION BY base_masset ORDER BY date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_sum
FROM tx
GROUP BY 1, 2
...
SELECT 
  ...
  sum(count(distinct sender)) OVER (ORDER BY date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_sum
FROM tx
GROUP BY 1, 2