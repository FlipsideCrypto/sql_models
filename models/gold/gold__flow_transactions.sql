...
SELECT
  ...
  CASE
    ...
    WHEN tx_type = 'deposit | withdraw' THEN 'nft_transfer' -- nft lifecycle behavior
    WHEN tx_type LIKE '%moment_minted%'
    OR tx_type LIKE '%token_minted%'
    OR tx_type LIKE '%nft_minted%' THEN 'nft_minted'
    WHEN tx_type LIKE '%moment_listed%'
    OR tx_type LIKE '%token_listed%'
    OR tx_type LIKE '%nft_listed%' THEN 'nft_listed'
    WHEN tx_type LIKE '%moment_withdrawn%'
    OR tx_type LIKE '%token_withdrawn%'
    OR tx_type LIKE '%nft_withdrawn%' THEN 'nft_withdrawn'
    WHEN tx_type LIKE '%moment_purchased%'
    OR tx_type LIKE '%token_purchased%'
    OR tx_type LIKE '%nft_purchased%' THEN 'nft_purchased'
    ...
    WHEN tx_type = 'new_series_started' THEN 'new_series_started' -- staking related
    ...
    WHEN tx_type LIKE '%rewards_paid%' THEN 'rewards_paid' -- account/contract related
    WHEN tx_type = 'account_key_added' THEN 'account_key_added'
    WHEN tx_type LIKE '%account_created%' THEN 'account_created'
    WHEN tx_type LIKE '%contract_initialized%' THEN 'contract_initialized' -- other
    WHEN tx_type LIKE '%tokens_minted%' THEN 'tokens_minted'
    WHEN tx_type LIKE '%tokens_burned%' THEN 'tokens_burned' -- catch all for single-type events like unlock_limit_increased
