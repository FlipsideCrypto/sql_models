{{ config(
  ...
  unique_key = "CONCAT_WS('-', join_date, new_mirror_users)",
...
WHERE msg_value:contract = 'terra15gwkyepfc6xgca5t5zefzwy42uts8l2m4g40k6' -- contract ID for mirror
...
SELECT
  join_date,
  COUNT(sender_address) new_mirror_users -- Addresses are already unique
FROM
  join_date_per_mirror_user
WHERE join_date >= '2020-11-01' -- We'll focus on 2021
GROUP BY 1
ORDER BY 1